 ...
 */
export function parseServerString(text) {
    ...
    if (eqIndex === -1) {
      if (entry.startsWith("http://") || entry.startsWith("https://")) {
        servers.push({
          ...
          address: entry,
          label: getLabelFromAddress(entry),
        });
      }
      continue;
    }
...
export function parseServers() {
  const envVal = process.env.MCP_SERVERS;
  let servers = [];
  if (envVal && typeof envVal === "string" && envVal.trim()) {
    servers = parseServerString(envVal);
  }
  if (servers.length === 0) {
    servers = parseServerString(DEFAULT_MCP_SERVERS);
  }
  return servers;
}
